...
uint8_t a_star(Map *map, Path *path)
{
    ...
    uint16_t total_costs[MAX_HEIGHT][MAX_WIDTH] = {{0}};
    ...
    for (uint8_t i = 0; i < map->height; i++)
    {
        ...
        {
            distances[i][j] = 65535;
            total_costs[i][j] = 65535;
        }
    }

    // Set Start Distance to 0
    distances[map->start_y][map->start_x] = 0;
    uint8_t esitmated_distance = abs(map->end_x - map->start_y) + abs(map->end_y - map->start_y);
    total_costs[map->start_y][map->start_x] = distances[map->start_y][map->start_x] + esitmated_distance;
    ...
    while (1)
    {
            ...
            if (VALID_NODE(potential_x, potential_y, map->width, map->height) && map->map[potential_y][potential_x] != '#')
            {
                ...
                if (visited[potential_y][potential_x] == 0)
                {
                    ...
                    if (distances[potential_y][potential_x] > distances[current_y][current_x] + 1)
                    {
                        ...
                        esitmated_distance = abs(map->end_x - potential_x) + abs(map->end_y - potential_y);
                        total_costs[potential_y][potential_x] = distances[potential_y][potential_x] + esitmated_distance;
                    }
                }
            }
        }
        ...
        uint16_t min_total_cost = 65535;
        for (uint8_t i = 0; i < map->height; i++)
        {
            for (uint8_t j = 0; j < map->width; j++)
            {
                if (visited[i][j] == 0 && total_costs[i][j] < min_total_cost)
                {
                    min_total_cost = total_costs[i][j];
                    current_x = j;
                    current_y = i;
                }
            }
