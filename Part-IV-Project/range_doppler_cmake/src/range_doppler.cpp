...
void RangeDoppler::azimuthFiltering(vector2D<complex<double>> &doppler_range_compressed_matrix_rcmc)
{
    ...
    vector2D<complex<double>> azimuth_filter_matrix = createAzimuthFilter(doppler_axis, false);
...
vector2D<std::complex<double>> RangeDoppler::createAzimuthFilter(const vector<double> doppler_axis, bool linearFMapprox)
{
    vector2D<std::complex<double>> out(mRows, mCols);
...
int RangeDoppler::loadParams(const std::string &filename)
{
    ...
    file.read(reinterpret_cast<char *>(&mRows), sizeof(mRows));                           // Read rows
    file.read(reinterpret_cast<char *>(&mCols), sizeof(mCols));                           // Read cols
